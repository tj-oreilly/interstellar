...
MIN_STEP = 5e-14  # Minimum velocity pc/s
MAX_STEP = 5e-14  # Maximum velocity pc/s
...
def take_stars_from_GAIA_data():
    ...
    for i, row in GAIA_data.iterrows():
        ...
        pmra = row['pmra']
        pmdec = row['pmdec']
        rv_kms = row['radial_velocity'] if 'radial_velocity' in row else 0

        distance_pc = 1000 / parallax
        mu_ra_arcsec = pmra / 1000
        mu_dec_arcsec = pmdec / 1000

        vx_tan = PM_CONVERSION * mu_ra_arcsec * distance_pc
        vy_tan = PM_CONVERSION * mu_dec_arcsec * distance_pc

        vx_list.append(vx_tan)
        vy_list.append(vy_tan)

    sun_velocity = pd.DataFrame({'vx [pc/s]': [0.0], 'vy [pc/s]': [0.0]})
    star_velocities = pd.DataFrame({'vx [pc/s]': vx_list, 'vy [pc/s]': vy_list}) * KMS_TO_PC_PER_S
...
def generate_stops(N_STOPS, stars_coordinates):
    ...
    random_stops = stars_coordinates.iloc[1:].sample(
        n=N_STOPS, random_state=25
    ).index.tolist()
...
def find_best_stop_order(selected_stops, step_size, star_coordinates, velocities_df):
    ...
    total_dist = sum(
        compute_distances_between_objects(
            *stars_coordinates.loc[stop_order[i]],
            *stars_coordinates.loc[stop_order[i + 1]]
        )
        for i in range(len(stop_order) - 1)
    )

    return stop_order, total_dist, C
...
def deflection_angle_from_path(path_indices, coordinates_df, velocities_of_stars):
    results = []
    ...
    for i in range(1, len(path_indices) - 1):
        ...
        # Get star velocity vector from velocities_df
        v_star = velocities_of_stars.loc[curr_idx, ['vx [pc/s]', 'vy [pc/s]']].values

        # Transform incoming and outgoing vectors to star frame
        vec_in_star_frame = vec_in - v_star
        vec_out_star_frame = vec_out - v_star

        # Normalize
        vec_in_unit = vec_in_star_frame / np.linalg.norm(vec_in_star_frame)
        vec_out_unit = vec_out_star_frame / np.linalg.norm(vec_out_star_frame)
...
def r0_periapsis_function(path_indices, parameters_df, deflection_angles_deg, velocity_magnitudes_list):
    ...
    for i in range(1, len(path_indices) - 1):
        ...
        v = velocity_magnitudes_list[i - 1] * PC_TO_M

        star_mass_solar = parameters_df.loc[idx, 'mass_flame']
        M = star_mass_solar * SOLAR_MASS
...
def uncertainties_deflection_angles(path_indices, parameters_df, n_samples=1000):
    results = []
    ...
    for i in range(1, len(path_indices) - 1):
        ...
        if a_idx == 0:
            a_samples = np.tile(np.array([[0.0, 0.0]]), (n_samples, 1))
        else:
...
def uncertainties_r0_periapsis(
    ...
    for i, (idx, deflection_deg) in enumerate(deflection_data):
        ...
        sigma_θ = np.radians(angle_uncertainties[i][1])

        v_pc_s = np.linalg.norm(velocities[i])
        v = v_pc_s * PC_TO_M
        sigma_v = velocity_uncertainties[i][2] * PC_TO_M

        M_solar = parameters_df.loc[idx, 'mass_flame']
        M_upper = parameters_df.loc[idx, 'mass_flame_upper']
        M_lower = parameters_df.loc[idx, 'mass_flame_lower']
        sigma_M = ((M_upper - M_lower) / 2) * SOLAR_MASS
        ...
        sigma_r0_km = np.sqrt(sigma_r0_squared) / 1e3

        results.append((idx, sigma_r0_km))
...
def simulate_probe_path(step_size: float, space_size: float, velocities_df):
    stops = generate_stops(N_STOPS, stars_coordinates)
    best_order, total_dist, hk_table = find_best_stop_order(
        stops, step_size, stars_coordinates, velocities_df
    )
    ...
    for d, v in zip(segment_distances, velocity_magnitudes):
        t = d / v
        observer_time_segments.append(t)
        gamma = 1 / np.sqrt(1 - (v ** 2 / SPEED_OF_LIGHT ** 2))
        proper_time_segments.append(t / gamma)
    ...
    print(
        f"Initial_velocity = {step_size} pc/s\n" 
        f"Travel time (observer): {observer_time:.2f} s or {observer_years:.2f} yrs \n"
        f"Travel time (probe):, {proper_time:.2f} s or {probe_years:.2f} yrs"
        )
    ...
    with open(f"Logs/Simulation_PF_STEP_SIZE={step_size}.txt", "w") as f:
        ...
        f.write(f"Initial velocity of the ship (Sun's frame): {step_size * PC_PER_S_TO_KM_PER_H:.3e} km/h\n")
        f.write(f"Final velocity (after assists) in Sun's frame: {final_velocity * PC_PER_S_TO_KM_PER_H:.3e} km/h\n")
        ...
        for (subset, end), (cost, path_idx) in hk_table.items():
            ...
            path_str = " → ".join(str(best_order[i]) for i in path_idx)
            f.write(
        ...
        # Convert list of (idx, σ_r0)
        r0_uncertainty_dict = dict(r0_uncertainties)

        for i, ((idx, r0), (idx, angle), v_vec, gain) in enumerate(zip(r0_periapsis, deflection_angles, velocities, velocity_gains), start=1):
            angle_stat_error = angle_uncertainties[i - 1][1]
            vx_stat_error, vy_stat_error, v_stat_error = velocity_uncertainties[i - 1]
            r0_stat_error = r0_uncertainty_dict[idx]

            v_kmh = np.linalg.norm(v_vec) * PC_PER_S_TO_KM_PER_H
            gain_km_h = gain * PC_PER_S_TO_KM_PER_H
            v_stat_error_kmh = v_stat_error * PC_PER_S_TO_KM_PER_H

            f.write(
                f"\n STOP {i+1} (index {idx}):\n" 
                f"      deflection_angle = {angle:.3f} ± {angle_stat_error:.3f} deg;\n"
                f"      r0 = {r0:.4e} ± {r0_stat_error:.2e} km;\n"
                f"      |v| = {v_kmh:.3e} ± {v_stat_error_kmh:.3e} km/h;\n"
                f"      σ_vx = {vx_stat_error:.3e} km/h, σ_vy = {vy_stat_error:.3e} km/h\n"
                f"      velocity_gain_Δv = {gain_km_h:.3e} km/h;\n"
                f"      Distance_traveled: {segment_distances[i]} pc;\n"
                f"      Travel time (observer): {observer_time_segments[i]:.2f} s or {observer_time_segments[i] / SECONDS_IN_YEAR:.2f} yrs;\n"
                f"      Travel time (probe):, {proper_time_segments[i]:.2f} s or {proper_time_segments[i] / SECONDS_IN_YEAR:.2f} yrs;\n"
                )
...
def plotting_function(
    ...
    # Draw path line
    coords = stars_coordinates.loc[best_order]
    ax.plot(
        coords["x [pc]"],
        coords["y [pc]"],
...
def create_space(step_size, radius, space_size, stars_velocities):
    fig = plt.subplots(figsize=(10, 8), facecolor='black')
    ax = plt.gca()
    sc = setup_star_map(ax, radius)

    if N_STOPS != 0:
        result = simulate_probe_path(step_size, space_size, stars_velocities)
        if result is None:
            return

        best_order = result
        plotting_function(ax, sc, step_size, best_order, space_size)
...
def main():
    ...
    for step in STEP_SIZES:
        create_space(step, radius, space_size, stars_velocities)
